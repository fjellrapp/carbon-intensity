...
async function getDefaultRegionalData() {
  return await getRegionalByName("england");
}

export default async function Home() {
  const region = await getDefaultRegionalData();
  ...
  return (
    <main className="grid grid-cols-4 row-auto gap-12 md:grid-cols-8 lg:grid-cols-12">
      <div className="my-12 col-span-full place-self-center">
        <h1 className="text-5xl font-bold md:text-6xl ">Carbon intensity</h1>
      </div>
      <CarbonIntensity defaultData={region} />
    </main>
  );
}
